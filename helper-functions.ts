...
import { AdminDetails, BookingDetails, CompanyDetails, ExpensesDetails, PropertiesDetails,GetBookingStats,BookingDetailsResponse } from "@/types";
...
//admin details (by default any user that signs up to the app is stored under this stable so it is similar to a user table for normal apps)
export async function getAdminDetails(id: string): Promise<AdminDetails> {
  ...
  return {
    ...
    email: data.email,
    company_uid: Array.isArray(data.company_uid) ? data.company_uid : [], // Ensure it's always an array
    photo_url: data.photo_url,
  };
}

export async function updateAdminDetails(id: string, updates: Partial<AdminDetails>): Promise<AdminDetails> {
  const response = await fetch(`/api/admin/${id}`, {
    method: "PATCH",
    headers: {
...
export async function insertAdminDetails(details: AdminDetails): Promise<AdminDetails> {
...
export async function updateBookingDetails(id: string, updates: Partial<BookingDetails>): Promise<BookingDetails> {
...
export async function insertBookingDetails(details: Partial<BookingDetails>): Promise<BookingDetails> {
...
export async function updateCompanyDetails(id: string, updates: Partial<CompanyDetails>): Promise<CompanyDetails> {
...
export async function insertCompanyDetails(details: CompanyDetails): Promise<CompanyDetails> {
...
export async function getCompanyDetails(id: string): Promise<CompanyDetails> {
  ...
  return {
    ...
    uid: data.uid,
    plan:data.plan,
    managers_email: Array.isArray(data.managers_email) ? data.managers_email : [], // Ensure it's always an array
  };
}

//expenses details (This gives  all the functions to interact with the expenses table )

export async function updateExpensesDetails(id: string, updates: Partial<ExpensesDetails>): Promise<ExpensesDetails> {
...
export async function insertExpensesDetails(details: ExpensesDetails): Promise<ExpensesDetails> {
...
export async function getExpensesDetails(
  ...
  if (!limit) queryParams.append("limit", "50");
  if (company_id) queryParams.append("company_id", company_id);
  if (created_by) queryParams.append("created_by", created_by);

  // Append query parameters only if they exist
  const url = `/api/expenses/${id}${queryParams.toString() ? `?${queryParams}` : ""}`;

  const response = await fetch(url);

  if (!response.ok) {
    throw new Error(`Failed to fetch expenses details for ID: ${id}`);
  }
...
export async function updatePropertiesDetails(id: string, updates: Partial<PropertiesDetails>): Promise<PropertiesDetails> {
...
export async function insertPropertiesDetails(details: PropertiesDetails): Promise<PropertiesDetails> {
...
export async function getPropertiesDetails(
  ...
  // Append query parameters only if they exist
  const url = `/api/properties/${id}${queryParams.toString() ? `?${queryParams}` : ""}`;

  const response = await fetch(url);

  if (!response.ok) {
    throw new Error(`Failed to fetch properties details for ID: ${id}`);
  }

  const data = await response.json();

  // Ensure it handles an array of properties correctly
...
//function to get booking stats
  export async function getBookingStats(company_id: string, start_date: string, end_date: string): Promise<GetBookingStats> {
    ...
    try {
      ...
      // Handle errors
      if (!response.ok) {
        throw new Error(`Failed to fetch booking stats for range: ${start_date} - ${end_date}`);
      }
...
export async function getUserBookingDetails(
    ...
    try {
        const response = await fetch(`/api/booking_details?id=${companyId}&startDate=${startDate}&endDate=${endDate}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
            },
...
export async function getRevenueAndExpenses(companyId: string): Promise<{ month: string; revenue: number; expenses: number; }[] | null> {
  const startDate = `${new Date().getFullYear()}-01-01`;
  const endDate = `${new Date().getFullYear()}-12-31`;
  try {
      const response = await fetch(`/api/revenue?id=${companyId}&startDate=${startDate}&endDate=${endDate}`, {
          method: "GET",
          headers: {
              "Content-Type": "application/json",
          },
      ...
      const data: { month: string; revenue: number; expenses: number; }[] = await response.json();
      return data;
  } catch (error) {
